...
##### Moreover, due to the weak prediction of tree model, i use the second and third model to make a prediction on test set. And the following are the result. And the left one is linear discrimination model and right one is Tree model. 
```{r,echo=T}
library(ggplot2);library(gridExtra)
dat=data.frame(pTree,pLds)
g2=ggplot(dat, aes(pLds,fill=pLds))+geom_histogram()
g2=g2+xlab("Classe Levels, LDS Model ")+ylab("Frequency LDS Model");g2

g3=ggplot(dat, aes(pTree,fill=pTree))+geom_histogram()
g3=g3+xlab("Classe Levels, Tree Model")+ylab("Frequency Tree");g3
